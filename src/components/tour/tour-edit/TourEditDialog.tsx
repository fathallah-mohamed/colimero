...
import { tourFormSchema } from "./TourValidationSchema";
import { useTourEdit } from "./useTourEdit";
...
export function TourEditDialog({ isOpen, onClose, tour, onComplete }: TourEditDialogProps) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      departure_date: tour?.departure_date ? new Date(tour.departure_date) : new Date(),
