...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { email, firstName, activationCode, resend: isResend = false } = requestData as EmailRequest;
    ...
    const { data: emailData, error: emailError } = await resend.emails.send({
      ...
      to: email.trim(),
      subject: isResend ? 'Nouveau code d\'activation Colimero' : 'Activez votre compte Colimero',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #1a365d; text-align: center;">Bienvenue ${firstName || ''}!</h2>
          <p style="color: #4a5568; text-align: center;">
            ${isResend ? 'Voici votre nouveau code d\'activation' : 'Merci de vous être inscrit sur Colimero'}. Pour activer votre compte, veuillez utiliser le code suivant :
          </p>
