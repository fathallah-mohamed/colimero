...
const handler = async (req: Request): Promise<Response> => {
  try {
    ...
    const activationUrl = `${req.headers.get("origin")}/activation?token=test-token-123`;
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: ["fathallahmohamed@hotmail.com"],
    ...
    console.error("Error in test-activation-email function:", error);
    return new Response(
